...
public class CmsServletExceptionHandler extends ResponseEntityExceptionHandler {
	...
	@ExceptionHandler(value = { CmsEmailAlreadyUsedException.class })
	protected ResponseEntity<Object> CmsInvalidRequestException(RuntimeException exception, WebRequest request) {
		return dispatchErrorHandling(HttpStatus.BAD_REQUEST.value(), ErrorCode.SEC_EMAIL_ALREADY_USED, exception,
				request);
	}

	@ExceptionHandler(value = { CmsAccountLockedException.class })
	...
	@ExceptionHandler(value = { CmsMissingValidationException.class })
	protected ResponseEntity<Object> handleCmsMissingValidationException(RuntimeException exception,
			WebRequest request) {
		return dispatchErrorHandling(HttpStatus.UNAUTHORIZED.value(), ErrorCode.SEC_MISSING_VALIDATION, exception,
				request);
	}

	@ExceptionHandler(value = { CmsAuthenticationException.class })
	protected ResponseEntity<Object> handleCmsAuthenticationException(RuntimeException exception, WebRequest request) {
		return dispatchErrorHandling(HttpStatus.UNAUTHORIZED.value(), ErrorCode.SEC_INVALID_CREDENTIALS, exception,
				request);
	}

	@ExceptionHandler(value = { CmsEntryNotFoundException.class })
	...
	@ExceptionHandler(value = { CmsMessagingUnavailableException.class })
	protected ResponseEntity<Object> handleCmsMessagingException(RuntimeException exception, WebRequest request) {
		return dispatchErrorHandling(HttpStatus.SERVICE_UNAVAILABLE.value(), ErrorCode.HAN_MESSAGING_UNAVAILABLE,
				exception, request);
	}

	@ExceptionHandler(value = { CmsPasswordRequirementsNotMet.class })
	protected ResponseEntity<Object> handleCmsPasswordRequirementsNotMet(RuntimeException exception,
			WebRequest request) {
		return dispatchErrorHandling(HttpStatus.BAD_REQUEST.value(), ErrorCode.SEC_PASSWORD_REQUIREMENTS_NOT_MET,
				exception, request);
	}

	private ResponseEntity<Object> dispatchErrorHandling(int httpStatusValue, ErrorCode errorCode,
